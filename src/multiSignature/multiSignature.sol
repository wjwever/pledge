...
contract multiSignature  is multiSignatureClient {
    ...
    constructor(address[] memory _owners,uint256 limitedSignNum) multiSignatureClient(address(this)) {
        require(_owners.length>=limitedSignNum,"Multiple Signature : Signature threshold is greater than owners' length!");
        signatureOwners = _owners;
